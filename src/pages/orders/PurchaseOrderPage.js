...
export default function PurchaseOrderPage() {
    ...
    const getVouchers = async () => {
        const res = await axios.get(`${'http://62.72.57.19:5000'}/api/voucher/viewVoucherDetails/${id}`);
        console.log("my voucher response", res)
        setVouchers(res.data.data);

    }
    const updateVoucher = async (voucherId) => {
        const res = await axios.get(`${'http://62.72.57.19:5000'}/api/voucher/addVoucherAdditionalData/${voucherId}`);
    ...
    const savePurchase = (id) => {
        ...
        axios.post(`${'http://62.72.57.19:5000'}/api/voucher/addVoucherDetails/${id}`, newVoucherData).then(res => console.log("my res", res));
